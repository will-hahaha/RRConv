...
class RectConv2d(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=True, flag=False, l_max=30,
                 w_max=18):
        super(RectConv2d, self).__init__()
        self.lmax = l_max
        self.wmax = w_max
        self.nmax = (l_max // 4) * (w_max // 4)
        self.inc = inc
        self.outc = outc
        ...
        self.p_conv[2].register_full_backward_hook(self._set_lr)
        ...
        if modulation:
            self.m_convs = nn.ModuleList([
                nn.Conv2d(inc, i, kernel_size=3, padding=1, stride=stride, bias=False)
                for i in range(1, self.nmax + 1)
            ])
            self.m_convs.register_full_backward_hook(self._set_lr)
    ...
    def forward(self, x):
        print("epoch: ", self.epoch, "last_N_X:", self.last_N_X, "last_N_Y:", self.last_N_Y)
        bias = self.b_conv(x)
        offset = self.p_conv(x)

        l = self.l_sig(self.l_conv(offset)) * (self.lmax - 1.5) + 1.5
        w = self.w_sig(self.w_conv(offset)) * (self.wmax - 1.5) + 1.5
