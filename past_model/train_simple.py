...
class RectConv2d(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=True, flag=False, l_max=7, w_max=7):
        ...
        self.m_conv = nn.Conv2d(inc, outc, kernel_size=3, padding=1, stride=stride)
    ...
    def forward(self, x):
        ...
        l = self.l_sig(l) * (self.lmax - 1) + 1 # b, 1, h, w
        w = self.w_sig(w) * (self.wmax - 1) + 1  # b, 1, h, w
        theta = self.theta_sof(theta) * 3.1415926  # b, 1, h, w
