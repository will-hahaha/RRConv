...
class RectConv2d(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=True, flag=False, l_max=70,
                 w_max=50):
        super(RectConv2d, self).__init__()
        self.lmax = l_max
        self.wmax = w_max
        self.nmax = (l_max // 10) * (w_max // 10)
        self.inc = inc
        self.outc = outc
        ...
        self.p_conv = nn.Sequential(
            ...
            mySin(),
            nn.Dropout(0),
            nn.Conv2d(3, 3, kernel_size=3, padding=1, stride=stride),
            nn.BatchNorm2d(3)
        )

        self.l_sig = nn.Sigmoid()
        self.w_sig = nn.Sigmoid()
        self.theta_sof = nn.Softsign()

        self.b_conv[0].register_backward_hook(self._set_lr)
        self.m_conv[0].register_backward_hook(self._set_lr)
        self.p_conv[0].register_backward_hook(self._set_lr)
        self.p_conv[2].register_backward_hook(self._set_lr)
    ...
    def forward(self, x):
        ...
        l = self.l_sig(l) * (self.lmax - 3) + 3
        w = self.w_sig(w) * (self.wmax - 3) + 3
        theta = self.theta_sof(theta) * 3.1415926
