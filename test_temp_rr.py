...
from model_temp_for_test import RECTNET
import numpy as np
import h5py
...
model.load_state_dict(torch.load('path1/checkpoint_290_2024-09-26-05-48-30.pth')['model'])
model.eval()
file_path = "test_data/test_wv3_multiExm1.h5"
test_gt, test_lms, test_ms, test_pan = load_set(file_path)

if  os.path.exists("1mats/x_1.mat"):
    print("Loading 1mats")
    nx, ny = [0 for _ in range(10)], [0 for _ in range(10)]
    tensor = [torch.tensor(sio.loadmat(f"1mats/x_{i}.mat")['x']).cuda() for i in range(1, 11)]
    for i in range(10):
